...
export const BrowseFilter = ({ query }: BrowseFilterProps) => {
  ...
  const initialSort = isValidSortParam(initialSortUnchecked) ? initialSortUnchecked : config.RECIPE_PAGINATION_DEFAULT_SORT;

  const [search, setSearch] = useState(initialSearch);
  const [sort, setSort] = useState<Sort>(sorts.find((sort) => getSortKey(sort) === initialSort) || sorts[0]);
  ...
  const newUrl = "/browse" + (searchParams.size > 0 ? "?" + searchParams.toString() : "");
